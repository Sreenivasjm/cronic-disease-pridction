...
def predictact():
   if request.method == 'POST':    
    ...
    if B_pred == 1:
        result="The patient has Chronic Kidney Disease"
        print("cronical kidney disease detected")
    if B_pred == 0:
        result="The patient has no Chronic Kidney Disease"
...
def viewd():
    global row
    data=pd.read_csv("C:\\Users\\Windows\\cronic disease detection using ML\\dataset\\chronic_final.csv")
    peek=data.head(100)
